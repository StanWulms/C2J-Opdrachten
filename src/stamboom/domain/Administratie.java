...
public class Administratie implements java.io.Serializable{
    ...
    public Persoon addPersoon(Geslacht geslacht, String[] vnamen, String anaam,
        ...
        for(int i = 0; i < vnamen.length; i++){
            vnamen[i] = vnamen[i].trim().substring(0,1).toUpperCase() + vnamen[i].trim().substring(1).toLowerCase();
        }
        
        anaam = anaam.trim().substring(0,1).toUpperCase() + anaam.trim().substring(1).toLowerCase();
        
        gebplaats = gebplaats.trim().substring(0,1).toUpperCase() + gebplaats.trim().substring(1).toLowerCase();
        tvoegsel = tvoegsel.trim().toLowerCase();
        
        //check data
        if(this.getPersoon(vnamen, anaam, tvoegsel, gebdat, gebplaats)!=null){
            return null;//person already exists
        }
    ...
    public Persoon getPersoon(int nr) {
        ...
        if(nr<0||nr>=nextPersNr){
            return null;//out of range
        }
        //return first number with nr = getNr()
    ...
    public ArrayList<Persoon> getPersonenMetAchternaam(String achternaam) {
        //todo opgave 1
        //return (ArrayList)personen.stream().filter((pers) -> pers.getAchternaam().equalsIgnoreCase(achternaam))
        //        .collect(Collectors.toList());
        ArrayList<Persoon> pwa = new ArrayList<>();
        for(Persoon pers : personen){
            
            if(pers.getAchternaam().equalsIgnoreCase(achternaam.trim())){
                pwa.add(pers);
            }
        }
        return pwa;
    ...
    public Persoon getPersoon(String[] vnamen, String anaam, String tvoegsel,
        ...
        List<Persoon> pma = getPersonenMetAchternaam(anaam);
        
        if(pma.size()<1){
            return null;//nobody found
        }
        for(Persoon p : pma){
            if(vnamenFormater(vnamen).equalsIgnoreCase(p.getInitialen())&&
                    tvoegsel.equalsIgnoreCase(p.getTussenvoegsel())&&
                    gebdat.equals(p.getGebDat()) && 
                    gebplaats.equalsIgnoreCase(p.getGebPlaats())){
                return p;//found m
            }
        }
        return null;
    ...
    private String vnamenFormater(String[] voornamen){
    ...
    public List<Gezin> getGezinnen() {
        return (List<Gezin>)Collections.unmodifiableList(gezinnen);
    }
