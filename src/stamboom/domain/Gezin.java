...
public class Gezin {
    ...
    boolean setHuwelijk(Calendar datum) {
        if(this.huwelijksdatum == null || this.scheidingsdatum != null){
    ...
    public String beschrijving() {
        ...
        if(kinderen.size()>0){
            ...
            kinderen.stream().forEach((kind) -> {
                builder.append(" -").append(kind.getVoornamen());
            });
        }
    ...
    public boolean isHuwelijkOp(Calendar datum) {
        if(this.huwelijksdatum == null)return false;
        if(this.huwelijksdatum.before(datum)) return true;
